...
static void transpose_submit(size_t M, size_t N, double A[N][M], double B[M][N],
                             double tmp[TMPCOUNT]) {
    // test on M = 32, N = 32 and M = 1024, N = 1024
    unsigned long block = TMPCOUNT / 32;
    if (M % block == 0 && N % block == 0) {
        unsigned long i, j;
        unsigned long i_c, j_c;
        // control loops
        for (i_c = 0; i_c < M; i_c += block)
            for (j_c = 0; j_c < N; j_c += block) {
                for (i = i_c; i < i_c + block; i++) {
                    for (j = j_c; j < j_c + block; j++) {
                        if (i == j) {
                            tmp[0] = A[i][j]; // i = j means i_c=j_c and they
                                              // incremented by the same step
                        } else {
                            B[j][i] = A[i][j];
                        }
                    }
